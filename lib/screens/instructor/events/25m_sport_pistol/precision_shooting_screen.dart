...
import 'precision_session_report_screen.dart';
...
class PrecisionShootingScreen extends StatefulWidget {
  ...
  @override
  State<PrecisionShootingScreen> createState() => _PrecisionShootingScreenState();
}

class _PrecisionShootingScreenState extends State<PrecisionShootingScreen> {
...
void _confirmCurrentShot() {
  ...
  if (!shot.isConfirmed) {
    // ✅ CHECK MAXIMUM SHOTS LIMIT (30 for Sports Pistol)
    if (!_isSightingMode) {
      ...
      if (confirmedCount >= 29) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Maximum 30 shots reached for 25m Precision Pistol'),
            backgroundColor: Color(0xFFD32F2F),
            duration: Duration(seconds: 2),
          ),
        );
        return; // Stop here - don't confirm more shots
      }
    }
...
void _calculateScore(PrecisionShot shot) {
  ...
  // ✅ Scale factor matching the painter: 500mm outer diameter
  final scale = _targetSize / 500.0;
  ...
  // Check if completely outside ring 1 (outermost ring)
  ...
  for (int ring = 10; ring >= 1; ring--) {
    ...
    if (pelletInnerEdgeDistance <= outerRingRadius) {
      ...
      if (ring == 10) {
        ...
        // Rings 9-1: score with decimal (e.g., 9.0 to 9.9)
...
final reportData = PrecisionSessionReportData(
  ...
  totalScore: _calculateTotalScore,
  totalTime: _totalSessionTime,
  eventType: '25m Precision Pistol',
  notes: finalNotes,
                                ...
                                child: Stack(
                                  children: [
                                    CustomPaint(
                                      ...
                                      painter: PrecisionPistolTargetPainter(
...
class PrecisionPistolTargetPainter extends CustomPainter {
  final List<PrecisionShot> shots;
  final int currentShotIndex;
  final double targetSize;
  final int shotsPerBatch;

  PrecisionPistolTargetPainter({
    required this.shots,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // ✅ Scale factor: 500mm outer diameter (ring 1) scaled to targetSize
    final scale = targetSize / 500.0;
    ...
    final double innerTenRadius = (25.0 / 2.0) * scale; // Inner 10-ring (25mm radius = 50mm diameter)
    ...
    // ✅ FIXED: Inner 10-ring drawn as RING (stroke only), not filled
    canvas.drawCircle(
      center,
      innerTenRadius,
      Paint()
  ...
  @override
  bool shouldRepaint(covariant PrecisionPistolTargetPainter oldDelegate) => true;
}
