...
class KneelingShootingScreen extends StatefulWidget {
  ...
  @override
  State<KneelingShootingScreen> createState() => _KneelingShootingScreen();
}

class _KneelingShootingScreen extends State<KneelingShootingScreen> {
...
void _confirmCurrentShot() {
  ...
  if (!shot.isConfirmed) {
    // ✅ MAXIMUM 20 SHOTS LIMIT CHECK
    if (!_isSightingMode) {
      final confirmedCount = _shots.where((s) => s.isConfirmed).length;
      if (confirmedCount >= 19) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Maximum 20 shots reached'),
...
void _calculateScore(PrecisionShot shot) {
  ...
  // ✅ Scale factor matching the painter: 154.4mm outer diameter
  final scale = _targetSize / 154.4;
  
  final pelletRadius = 5.6 / 2 * scale; // 5.6mm for .22 LR

  // ✅ 50m Rifle 3P Target - Ring DIAMETERS in mm
  ...
  final ring10Radius = ringRadii[10]!; // Outer 10-ring (5.2mm radius)
  final innerTenRadius = 2.6 * scale;  // Inner 10-ring (half of 5.2mm = 10.4 boundary)
  ...
  // Check if completely outside ring 1
  ...
  // ✅ RING 10 SCORING WITH INNER/OUTER ZONES
  if (pelletInnerEdgeDistance <= ring10Radius) {
    ringNumber = 10;
    
    if (shotCenterDistance <= innerTenRadius) {
      // ✅ INSIDE INNER 10 ZONE: 10.4 to 10.9
      final distanceRatio = shotCenterDistance / innerTenRadius;
      final scoreRatio = 1.0 - distanceRatio;
      final decimal = 0.4 + (scoreRatio * 0.5);
      score = 10.0 + decimal.clamp(0.4, 0.9);
    } else {
      // ✅ BETWEEN INNER 10 & OUTER 10: 10.0 to 10.4
      final zoneWidth = ring10Radius - innerTenRadius;
      final distanceFromInnerCircle = pelletInnerEdgeDistance - innerTenRadius;
      
      if (distanceFromInnerCircle <= 0) {
        score = 10.4;
      } else {
  ...
  // Rings 9 down to 1: standard scoring
  ...
  Future<void> _showLightSelector() async {
    final result = await showDialog<String>(
      ...
      builder: (context) => SimpleOptionDialog(
        ...
        color: const Color.fromARGB(255, 120, 78, 16),
      ),
    );
  ...
  Future<void> _showClimateSelector() async {
    final result = await showDialog<String>(
      ...
      builder: (context) => SimpleOptionDialog(
        ...
        color: const Color.fromARGB(255, 42, 94, 89),
      ),
    );
        ...
        title: Row(
          children: [
            Container(
              ...
              child: Text(
                _isSightingMode ? 'Sighting' : 'Precision',
                style: const TextStyle(
                  color: Colors.white,
                  fontSize: 11,
                  fontWeight: FontWeight.bold,
...
Row(
  ...
  children: [
    _buildSmallIndicatorButton(
      label: _getLightIcon(),
      color: const Color(0xFFFFA726), // Soft amber - warm light theme[file:21]
      onTap: _showLightSelector,
    ),
    const SizedBox(width: 8),
    _buildSmallIndicatorButton(
      label: _getWindDirectionIcon(),
      color: const Color(0xFF42A5F5), // Clean sky blue (unchanged)
      onTap: _showWindDirectionSelector,
    ),
    const SizedBox(width: 8),
    _buildSmallIndicatorButton(
      label: _getClimateIcon(),
      color: const Color(0xFF4DB6AC), // Muted teal - weather perfect[file:22]
...
class Rifle3PTargetPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // ✅ Scale factor: 154.4mm outer diameter (ring 1) scaled to targetSize
    final scale = targetSize / 154.4;
    ...
    // ✅ Black ring diameter (from spec: "Black from part of 3 to 10 rings = 112.4mm")
    final blackAreaRadius = (112.4 / 2) * scale;

    // ✅ Draw rings 1-3 (white rings with black borders)
    for (int ringNum = 1; ringNum <= 3; ringNum++) {
    ...
    // ✅ Draw BLACK AREA (112.4mm diameter - covers part of ring 3 to ring 10)
    canvas.drawCircle(
      center,
      blackAreaRadius,
    ...
    // ✅ Draw rings 4-10 with WHITE borders (on black background)
    for (int ringNum = 4; ringNum <= 10; ringNum++) {
      final radius = ringRadii[ringNum]!;
    ...
    for (int ringNum = 1; ringNum <= 8; ringNum++) {
      ...
      // ✅ Ring 3 text color depends on position (black part or white part)
    ...
    // ✅ INNER 10-RING (5mm diameter = 2.5mm radius)
  ...
  @override
  bool shouldRepaint(covariant Rifle3PTargetPainter oldDelegate) => true;
}
