...
Widget _buildInvisibleShotsTable() {
  return RepaintBoundary(
    key: _shotsTableKey,
    child: Material( // ✅ Add Material wrapper
      ...
      child: DefaultTextStyle( // ✅ Add DefaultTextStyle
        ...
        child: 
    Container(
      color: const Color(0xFF1A1A1A), // BLACK background for entire table
      width: 1400,
      padding: const EdgeInsets.all(16),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
...
List<Widget> _buildTableRowsForPDF() {
  ...
  for (int i = 0; i < widget.reportData.shots.length; i++) {
    ...
    rows.add(_buildPdfShotRow(shot, currentShotNumber));
...
Widget _buildPdfShotRow(Map<String, dynamic> shot, int shotNumber) {
...
// NOTES SECTION - Display all notes in reverse chronological order
                  if (widget.reportData.notesList != null && widget.reportData.notesList!.isNotEmpty)
                    Container(
                                ...
                                children: [
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      Text(
                                        DateFormat('dd MMM yyyy, HH:mm').format(note.timestamp),
                                        style: TextStyle(
                                          color: Colors.white.withOpacity(0.5),
                                          fontSize: 12,
                  ...
                  else if (widget.reportData.notes != null && widget.reportData.notes!.trim().isNotEmpty)
                    Container(
                      margin: const EdgeInsets.only(bottom: 16),
                      padding: const EdgeInsets.all(12),
                      decoration: BoxDecoration(
                      ...
                      child: Row(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Icon(Icons.sticky_note_2_outlined, color: Color(0xFFD32F2F), size: 20),
                      ...
                      child: Column(
                        children: [
                            ...
                            children: [
                              Column(
                                children: [
                                  ...
                                  const SizedBox(height: 4),
                                  const Text('Shots', style: TextStyle(color: Color(0xFFD32F2F), fontSize: 14, fontWeight: FontWeight.bold)),
                                ],
                              ),
                              Column(
                                children: [
                                  ...
                                  const SizedBox(height: 4),
                                  const Text('Score', style: TextStyle(color: Color(0xFFD32F2F), fontSize: 14, fontWeight: FontWeight.bold)),
                                ],
                              ),
                              Column(
                                children: [
                                  Text(
                                    _formatDurationWithMillis(widget.reportData.totalTime),
                                    style: const TextStyle(color: Colors.white, fontSize: 25, fontWeight: FontWeight.bold),
                                  ),
                                  const SizedBox(height: 4),
                                  const Text('Total\nTime', style: TextStyle(color: Color(0xFFD32F2F), fontSize: 12, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
                                ],
                              ),
                            ],
                          ),
                          ...
                          Row(
                            mainAxisAlignment: MainAxisAlignment.spaceAround,
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: _buildScoreBreakdownColumns(),
                          ),
                        ],
                      ),
                    ),
...
//   ),
                    ...
                    // ✅ SHARE BUTTON
// In your build method, add this section before the Share Report button:

// ===== ATTACHED FILES SECTION =====
// Display attached files if any exist
  ...
  Container(
    ...
    child: Column(
      ...
      children: [
        ...
        const SizedBox(height: 12),
// In your attached files display section, update the IconButton:
...
// ===== SHARE REPORT BUTTON (Updated) =====
Container(
  margin: const EdgeInsets.symmetric(horizontal: 16),
  width: double.infinity,
  child: ElevatedButton.icon(
    onPressed: attachedFiles.isEmpty 
        ? _generatePdf  // Direct PDF if no files attached
        : showShareOptionsDialog,  // Show options if files attached
...
const SizedBox(height: 24),
      ...
      child: Column(
        children: [
          ...
          _buildInvisibleScoreGraph(),  // ✅ ADD THIS
          _buildInvisibleCumulativeTarget(),  // ✅ NEW
          _buildInvisibleShotsTable(),     
        ],
      ),
      ),
    ],
...
List<Widget> _buildVisibleGroupCards() {
  ...
  return groups.asMap().entries.map((entry) {
    ...
    return Padding(
      ...
      child: buildGroupCardWithPhotos(group, groupIndex), // ✅ Changed from buildGroupCard to buildGroupCardWithPhotos
    );
  }).toList();
}
...
List<Widget> _buildTableRows(List<Map<String, dynamic>> shots, int startIndex) {
  ...
  for (int i = 0; i < shots.length; i++) {
    ...
    if (missedShotMap.containsKey(currentShotNumber)) {
      for (var missed in missedShotMap[currentShotNumber]!) {
        rows.add(_buildMissedShotRow(
          missed.shotNumber,
          List<String>.from(missed.feedback),
          missed.shotTime, // ✅ Pass the time now
        ));
      }
    }
            ...
            child: Padding(
              ...
              child: feedbackList.isEmpty
                ...
                : Wrap(
                    ...
                    runSpacing: 4,
                    alignment: WrapAlignment.center,
                    children: feedbackList.map((feedbackId) => Image.asset(
                      ShootingFeedbackIcons.getIconPath(feedbackId) ?? '',
                      width: 16,
                      height: 16,
                      errorBuilder: (context, error, stackTrace) {
